...
function simplifyErrorMessage(originalError, context) {
  ...
  if (originalError && originalError.message) {
    ...
    } else if (msg.includes("[google.rpc.errorinfo]")) {
      // Specific Google API error details
...
export const callGeminiAPI = async (prompt, isJsonOutput = false) => {
  ...
  try {
    ...
    const generationConfig = isJsonOutput ? {
    ...
    if (isJsonOutput && textOutput) {
      ...
      try {
        ...
        return {
          error: simplifyErrorMessage({
            message: `Failed to understand the helper's special message.`
          }, 'api')
        };
      }
    }
    return {
      text: !isJsonOutput ? textOutput : undefined,
      json: jsonParsedOutput
    };
  } catch (error) {
...
export const callGeminiAPIWithAudioAndText = async (textPrompt, audioBase64, audioMimeType, systemInstruction) => {
  const apiKey = getApiKey();
  if (!apiKey) {
    return {
      error: simplifyErrorMessage({
...
const getAiInstance = () => {
  if (!globalAiInstance) {
    ...
    globalAiInstance = new GoogleGenAI({
      apiKey
    });
  }
...
export const sendChatMessageStream = async (chat, messageParts, onChunk, onEnd, onError) => {
  try {
