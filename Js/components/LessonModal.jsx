...
// Added InterestingFactContentBlock
} from '../types';
import { CloseIcon, CheckIcon, LightBulbIcon, StarIcon } from '../constants'; // Added StarIcon
...
const LessonModal = ({
  ...
  const renderContentBlock = block => {
    ...
    switch (block.type) {
      case ContentBlockType.TEXT:
        ...
        return <div key={textBlock.id} className={`${baseBlockClasses}`}>
      ...
      case ContentBlockType.KANA_INFO:
        const kanaBlock = block;
        return <div key={kanaBlock.id} className={`${baseBlockClasses}`}>
            {kanaBlock.title && <h3 className="text-xl font-semibold text-sky-300 mb-3">{kanaBlock.title}</h3>}
            {kanaBlock.introduction && <div className="p-3 bg-slate-800/40 rounded-lg mb-4 border border-slate-700">
                <p className="text-sm text-gray-300">{kanaBlock.introduction}</p>
              </div>}
            <KanaTable characters={kanaBlock.characters} kanaSet={kanaBlock.kanaSet} onStartWritingPractice={onStartWritingPractice} />
          </div>;
      case ContentBlockType.VOCABULARY_LIST:
        const vocabBlock = block;
        return <div key={vocabBlock.id} className={`${baseBlockClasses}`}>
            {vocabBlock.title && <h3 className="text-xl font-semibold text-sky-300 mb-3">{vocabBlock.title}</h3>}
            <ul className="space-y-3">
              {vocabBlock.items.map(item => <li key={item.id} className="glassmorphic-card-inner p-3 rounded-md">
                  ...
                  {item.exampleSentence && <div className="mt-2 p-2 bg-black/20 rounded text-xs italic border border-slate-700">
      ...
      case ContentBlockType.GRAMMAR_EXPLANATION:
        ...
        return <div key={grammarBlock.id} className={`${baseBlockClasses}`}>
            ...
            <ul className="space-y-2">
              {grammarBlock.exampleSentences.map((ex, index) => <li key={ex.id || `gram-ex-${index}`} className="p-3 bg-slate-700/50 rounded-md border border-slate-600">
      ...
      case ContentBlockType.AI_TUTOR_STREAM:
        return <AiTutorStream key={block.id} block={block} />;
      case ContentBlockType.INTERESTING_FACT:
        const factBlock = block;
        const relatedVocabItems = factBlock.relatedVocabularyIds.map(id => getVocabularyItemById(id)).filter(item => item !== undefined);
        return <div key={factBlock.id} className={`${baseBlockClasses} border-indigo-500/50 bg-indigo-500/10`}>
            ...
            {relatedVocabItems.length > 0 && <div>
                <h4 className="text-md font-semibold text-indigo-300 mt-3 mb-2">Key Vocabulary in this Story:</h4>
                <ul className="list-disc list-inside space-y-1 pl-2">
                  {relatedVocabItems.map(item => <li key={item.id} className="text-sm text-indigo-200">
      ...
      case ContentBlockType.GEMINI_PRACTICE:
        ...
        return <div key={block.id} className={`${baseBlockClasses} border-orange-500/50 bg-orange-500/10`}>
            <h3 className="text-xl font-semibold text-orange-300 mb-3">Legacy Practice Block</h3>
            <p className="text-orange-200">This practice type is being replaced by the AI Tutor Stream. Content: {block.instructionText}</p>
          </div>;
      case ContentBlockType.PRONUNCIATION_PRACTICE:
        ...
        return <div key={block.id} className={`${baseBlockClasses} border-orange-500/50 bg-orange-500/10`}>
            ...
            <p className="text-orange-200">This practice type is being replaced by the AI Tutor Stream. Target: {block.targetPhraseJapanese}</p>
          </div>;
      case ContentBlockType.CULTURAL_NOTE:
        const culturalBlock = block;
        return <div key={culturalBlock.id} className={`${baseBlockClasses} border-yellow-500/50 bg-yellow-500/10`}>
      ...
      case ContentBlockType.FILL_IN_THE_BLANK_EXERCISE:
        return <FillInTheBlankExercise key={block.id} block={block} />;
      case ContentBlockType.WRITING_PRACTICE:
        return <WritingPracticeExercise key={block.id} block={block} />;
      default:
        const unknownBlock = block;
        console.warn(`Encountered an unsupported content block type: ${unknownBlock.type || 'unknown'}`);
        return <div key={unknownBlock.id || `unknown-block-${Math.random().toString(36).substring(7)}`} className={`${baseBlockClasses} border-red-500/50 bg-red-500/10`}>
  ...
  return <div className="fixed inset-0 bg-black/70 backdrop-filter backdrop-blur-md flex items-center justify-center p-2 sm:p-4 z-[100] overflow-y-auto">
      <div className="glassmorphic-card w-full max-w-3xl max-h-[90vh] flex flex-col font-sans !p-0">
        <header className="p-4 sm:p-5 border-b-2 border-slate-700/50 flex justify-between items-center sticky top-0 bg-slate-800/80 backdrop-filter backdrop-blur-lg z-10 rounded-t-lg">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-sky-300 transition-colors p-2 rounded-full hover:bg-slate-700 active:bg-slate-600" aria-label="Close lesson">
        ...
        <footer className="p-4 sm:p-5 border-t-2 border-slate-700/50 sticky bottom-0 bg-slate-800/80 backdrop-filter backdrop-blur-lg z-10 rounded-b-lg">
          ...
          onClose();
            ...
            </button> : <p className="text-center text-green-400 font-semibold flex items-center justify-center text-lg">
