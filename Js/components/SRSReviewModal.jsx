...
import { CloseIcon, CheckIcon, QuestionMarkCircleIcon, LightBulbIcon } from '../constants';
import AudioPlayButton from './AudioPlayButton';
const SRSReviewModal = ({
  isOpen,
  onClose,
  vocabularyToReview,
  srsData,
  ...
  const handleNext = knewIt => {
    ...
    setSessionStats(prev => ({
      ...prev,
      [knewIt ? 'known' : 'unknown']: prev[knewIt ? 'known' : 'unknown'] + 1
    }));
  ...
  return <div className="fixed inset-0 bg-black/50 backdrop-filter backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 z-[110] overflow-y-auto">
      <div className="glassmorphic-card w-full max-w-xl flex flex-col font-sans"> {/* Use custom class */}
        <header className="p-4 border-b border-white/10 flex justify-between items-center bg-black/40 backdrop-filter backdrop-blur-md rounded-t-lg"> {/* Match card radius */}
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-sky-300 transition-colors p-2 rounded-full hover:bg-white/10 active:bg-white/20" aria-label="Close review">
        ...
        <main className="p-6 flex-grow min-h-[280px] flex flex-col justify-center items-center text-gray-100"> {/* Default text color */}
          <div className="text-center">
            ...
            {showAnswer ? <div className="mt-2 space-y-1.5 text-gray-300">
                <p className="text-xl font-sans">{currentItem.romaji}</p>
                <p className="text-lg font-sans">{currentItem.meaning}</p>
                {currentItem.exampleSentence && <p className="text-sm italic mt-3 text-gray-400">
              ...
              </div> : <p className="text-gray-400 text-xl h-20 flex items-center justify-center">(Meaning hidden)</p>}
          </div>
        </main>
        ...
        <footer className="p-4 border-t border-white/10 bg-black/40 backdrop-filter backdrop-blur-md rounded-b-lg"> {/* Match card radius */}
          {showAnswer ? <div className="flex justify-around gap-4">
              <button onClick={() => handleNext(false)} disabled={isSessionEnd && currentIndex === shuffledVocab.length - 1} className="w-full bg-red-500/60 hover:bg-red-400/70 backdrop-filter backdrop-blur-sm border border-red-400/60 text-white font-semibold py-3.5 px-4 rounded-xl flex items-center justify-center text-lg transition-all duration-300 shadow-lg hover:shadow-red-400/30 disabled:opacity-60 disabled:cursor-not-allowed">
                <QuestionMarkCircleIcon className="h-6 w-6 mr-2" /> Didn't Know
              </button>
              <button onClick={() => handleNext(true)} disabled={isSessionEnd && currentIndex === shuffledVocab.length - 1} className="w-full bg-green-500/60 hover:bg-green-400/70 backdrop-filter backdrop-blur-sm border border-green-400/60 text-white font-semibold py-3.5 px-4 rounded-xl flex items-center justify-center text-lg transition-all duration-300 shadow-lg hover:shadow-green-400/30 disabled:opacity-60 disabled:cursor-not-allowed">
                <CheckIcon className="h-6 w-6 mr-2" /> Knew It!
              </button>
            </div> : <button onClick={() => setShowAnswer(true)} className="w-full bg-sky-500/60 hover:bg-sky-400/70 backdrop-filter backdrop-blur-sm border border-sky-400/60 text-white font-semibold py-3.5 px-4 rounded-xl flex items-center justify-center text-lg transition-all duration-300 shadow-lg hover:shadow-sky-400/30">
              <LightBulbIcon className="h-6 w-6 mr-2" /> Show Answer
            </button>}
          {isSessionEnd && <div className="text-center text-sky-300 mt-4 p-3 bg-black/30 rounded-lg border border-white/10">
