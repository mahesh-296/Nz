...
const DayCard = ({
  ...
  return <div className={`${cardBaseClasses} ${stateClasses} relative overflow-hidden group`} onClick={() => !isLocked && onSelectDay(dayInfo)} role={isLocked ? undefined : "button"} tabIndex={isLocked ? undefined : 0} aria-label={`Lesson ${dayInfo.day}: ${dayInfo.title}${isCompleted ? ' (Completed)' : ''}${isLocked ? ' (Locked)' : ''}`}>
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="relative flex-grow overflow-hidden" style={{
        maxHeight: '4rem' /* max-h-16, approx 3 lines of text-sm (1.25rem line-height) + fade */
      }}>
          <p className={`text-sm ${overviewColor} leading-normal`}> {/* Use leading-normal or leading-relaxed */}
            {dayInfo.overview}
          </p>
          {/* Fade Overlay */}
          <div className="absolute bottom-0 left-0 right-0 h-8" // h-8 (2rem) for a smoother fade
        style={{
          background: `linear-gradient(to bottom, transparent, ${getCardBackgroundColor()})`
        }} aria-hidden="true" />
        </div>
      
        {!isLocked && <button className={`mt-auto w-full text-sm py-2.5 px-3 rounded-md font-semibold transition-colors duration-200 shadow-md focus:outline-none focus:ring-2 focus:ring-opacity-50 ${buttonStyle}`} onClick={e => {
        e.stopPropagation();
        onSelectDay(dayInfo);
      }}>
            {buttonText}
          </button>}
        {isLocked && <p className="mt-auto text-center text-xs text-gray-500 p-2 bg-black/30 rounded-b-lg">Complete previous lessons to unlock.</p>}
      </div>
      {isNextNewDay && !isLocked && !isCompleted && <div className="absolute top-2 right-2 px-2 py-0.5 bg-sky-500 text-white text-xs font-bold rounded shadow-md">
           NEXT
         </div>}
    </div>;
};
