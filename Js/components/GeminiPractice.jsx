...
const GeminiPractice = ({
  ...
  const handleGeminiQuery = useCallback(async currentPrompt => {
    setIsLoading(true);
    setError(null);
    setGeminiData(null);
  ...
  const formatGeminiTextResponse = text => {
    if (!text) return null;
  ...
  return <div className="glassmorphic-card p-6 my-6 font-sans text-gray-100">
      ...
      {block.requiresUserInput && <div className="mb-4">
          <label htmlFor={`gemini-input-${block.id}`} className="block text-sm font-medium text-gray-300 mb-1.5">
            Your response:
          </label>
          <textarea id={`gemini-input-${block.id}`} value={userInput} onChange={e => setUserInput(e.target.value)} placeholder={block.placeholder || "Type your answer here..."} rows={3} className="w-full p-3 bg-slate-700/60 border border-slate-600 rounded-lg text-gray-100 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-shadow font-japanese" lang="ja" />
        </div>}

      <button onClick={handleSubmit} disabled={isLoading || block.requiresUserInput && !userInput.trim()} className="w-full flex items-center justify-center bg-sky-600 hover:bg-sky-700 border border-sky-700 disabled:bg-slate-600 disabled:text-gray-400 disabled:border-slate-500 text-white font-semibold py-2.5 px-4 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-opacity-50">
      ...
      {geminiData && <div className="mt-5 p-4 bg-slate-700/50 rounded-lg border border-slate-600">
          <h5 className="text-md font-semibold text-sky-300 mb-2">AI Response:</h5>
          {geminiData.text && <div className="text-gray-200 whitespace-pre-wrap leading-relaxed p-3 bg-black/30 rounded-md">
              {formatGeminiTextResponse(geminiData.text)}
            </div>}
          {geminiData.json && <pre className="text-gray-200 whitespace-pre-wrap bg-slate-800 p-3 rounded-md text-sm overflow-x-auto border border-slate-700">
