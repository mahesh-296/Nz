...
const PronunciationPractice = ({
        ...
        mediaRecorderRef.current.onstop = async () => {
          ...
          reader.onloadend = async () => {
            ...
            if (base64Audio) {
              ...
              const response = await callGeminiAPIWithAudioAndText(textPromptForContext, base64Audio, 'audio/webm', block.geminiSystemInstruction || defaultSystemInstruction);
              if (response.error) {
  ...
  return <div className="glassmorphic-card p-6 my-6 font-sans text-gray-100">
      ...
      <div className="flex flex-col items-center space-y-4 mt-4">
        {!isRecording && !isLoading && <button onClick={startRecording} className="w-full max-w-xs flex items-center justify-center bg-green-600 hover:bg-green-700 border border-green-700 text-white font-semibold py-3 px-4 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg">
            <SparklesIcon className="h-5 w-5 mr-2" /> Start Recording
          </button>}
        {isRecording && <button onClick={stopRecording} className="w-full max-w-xs flex items-center justify-center bg-red-600 hover:bg-red-700 border border-red-700 text-white font-semibold py-3 px-4 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg">
      ...
      {geminiResponse && geminiResponse.text && <div className="mt-5 p-4 bg-slate-700/60 rounded-lg border border-slate-600">
