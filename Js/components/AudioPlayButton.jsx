...
const AudioPlayButton = ({
  ...
  return <button onClick={e => {
    ...
    playAudio();
  }} className={`p-1.5 rounded-full hover:bg-white/10 active:bg-white/20 transition-colors ${isPlaying ? 'text-sky-200 animate-pulse' : 'text-sky-300'} ${className}`} aria-label={`Listen to ${textToSpeak}`} title={`Listen to "${textToSpeak}"`}>
