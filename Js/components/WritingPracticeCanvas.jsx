...
const WritingPracticeCanvas = ({
  ...
  const [loadedGuidanceImage, setLoadedGuidanceImage] = useState(null);
  useEffect(() => {
  ...
  return <div className="flex flex-col items-center space-y-4 font-sans">
      <canvas ref={canvasRef} onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={endDrawing} onMouseLeave={endDrawing} onTouchStart={startDrawing} onTouchMove={draw} onTouchEnd={endDrawing}
    // Canvas background is now drawn in drawGuidance. Border from Tailwind.
    className="border-2 border-white/20 rounded-xl cursor-crosshair shadow-lg" aria-label={characterToPractice ? `Drawing canvas for character ${characterToPractice}` : "Drawing canvas"} id={canvasId} role="img" />
      <button onClick={handleClearUserDrawing} className="bg-sky-500/60 hover:bg-sky-400/70 backdrop-filter backdrop-blur-sm border border-sky-400/60 text-white font-semibold py-2 px-5 rounded-lg flex items-center justify-center transition-all duration-300 shadow-md hover:shadow-lg" aria-controls={canvasId}>
