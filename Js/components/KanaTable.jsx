...
const KanaTable = ({
  ...
  return <div className="my-4 font-sans">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
        ...
        return <div key={kana.char} className={`glassmorphic-card-inner p-3 flex flex-col items-center text-center h-full rounded-lg shadow-md hover:shadow-lg transition-shadow ${canPractice ? 'cursor-pointer hover:bg-sky-700/40' : 'opacity-90'}`} onClick={canPractice ? () => onStartWritingPractice(kana) : undefined} role={canPractice ? "button" : undefined} tabIndex={canPractice ? 0 : undefined} aria-label={canPractice ? `Practice writing ${kana.char}` : `${kana.char} - ${kana.romaji}`} onKeyDown={canPractice ? e => {
          if (e.key === 'Enter' || e.key === ' ') onStartWritingPractice(kana);
        } : undefined}>
              ...
              {kana.exampleWord && <div className="mt-2 pt-2 border-t border-slate-600/50 w-full text-xs">
