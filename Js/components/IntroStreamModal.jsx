...
const IntroStreamModal = ({
  ...
  return <div className="fixed inset-0 bg-black/80 backdrop-filter backdrop-blur-lg flex items-center justify-center p-2 sm:p-4 z-[200] font-sans">
      <div className="glassmorphic-card w-full max-w-2xl max-h-[90vh] flex flex-col !p-0 shadow-2xl border-sky-500/30">
        <header className="p-4 sm:p-5 border-b-2 border-slate-700/50 flex justify-between items-center bg-slate-800/80 backdrop-filter backdrop-blur-lg rounded-t-lg">
          ...
          {currentSlideIndex === introSlides.length - 1 ? null :
        // Hide close button on last slide to encourage "Get Started"
        <button onClick={onClose} className="text-gray-400 hover:text-sky-300 transition-colors p-2 rounded-full hover:bg-slate-700 active:bg-slate-600" aria-label="Skip intro">
        ...
        <footer className="p-4 sm:p-5 border-t-2 border-slate-700/50 bg-slate-800/80 backdrop-filter backdrop-blur-lg rounded-b-lg flex flex-col sm:flex-row items-center justify-between space-y-3 sm:space-y-0 sm:space-x-4">
          <div className="flex items-center justify-center space-x-2">
            {introSlides.map((_, index) => <button key={index} onClick={() => setCurrentSlideIndex(index)} className={`w-3 h-3 rounded-full transition-all ${index === currentSlideIndex ? 'bg-sky-400 scale-125' : 'bg-slate-600 hover:bg-slate-500'}`} aria-label={`Go to slide ${index + 1}`} />)}
          </div>
          <div className="flex w-full sm:w-auto space-x-3">
            <button onClick={handlePrevious} disabled={currentSlideIndex === 0} className="flex-1 sm:flex-initial bg-slate-600 hover:bg-slate-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold py-2.5 px-6 rounded-lg transition-colors flex items-center justify-center shadow-md">
              <ChevronLeftIcon className="h-5 w-5 mr-1 sm:mr-2" /> Previous
            </button>
            <button onClick={handleNext} className={`flex-1 sm:flex-initial text-white font-semibold py-2.5 px-6 rounded-lg transition-colors flex items-center justify-center shadow-md ${currentSlideIndex === introSlides.length - 1 ? 'bg-green-500 hover:bg-green-600' : 'bg-sky-500 hover:bg-sky-600'}`}>
